...
public class CategoryFilter extends HttpFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if (!Validators.isName(request.getParameter("cat_name"))) {
			Map<String, String> map = new HashMap<>();
			map.put("error_message", "Invalid Name");
			map.put("error_section", "cat_name");
			SendResponse.send(map, response, gson);
		}

		else if (request.getParameter("cat_about").isBlank()) {
			Map<String, String> map = new HashMap<>();
			map.put("error_message", "Description Cannot Be Blank");
			map.put("error_section", "cat_about");
			SendResponse.send(map, response, gson);
		}

		else if (!Validators.isFile(((HttpServletRequest) (request)).getPart("cat_profile").getSubmittedFileName())) {
			Map<String, String> map = new HashMap<>();
			map.put("error_message", "Invalid File Type");
			map.put("error_section", "cat_profile");
			SendResponse.send(map, response, gson);
		}
